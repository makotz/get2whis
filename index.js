...
function saveAndQuery(sender, conditions, userProfile) {
    ...
    if (wholeProfile['drive_or_ride'] == "looking_for_riders") {
        pg.connect(process.env.DATABASE_URL, function(err, client, done) {
          ...
          var potentialRiders = client.query('SELECT (first_name, last_name, profile_pic, departure_date, departure_time) FROM rider WHERE departure_location ='+wholeProfile.departure_location+'AND departure_date = '+wholeProfile.departure_date+'AND departure_time = '+wholeProfile.departure_time)
      ...
      pg.connect(process.env.DATABASE_URL, function(err, client, done) {
        ...
        console.log("potentialRiders are "+JSON.parse(potentialDriver));
      });
    }
};
